...
from jepa_control.models.modules import ACBlock as Block
from jepa_control.models.modules import build_action_block_causal_attention_mask
...
class VisionTransformerPredictorAC(nn.Module):
    ...
    def forward(self, x, actions, states, extrinsics=None):
        """
        :param x: context tokens [B, N_ctxt, D] or [B, T, HW, D]
        """
        if x.ndim == 4:
            x = x.flatten(1, 2)
        if actions.ndim == 2:
